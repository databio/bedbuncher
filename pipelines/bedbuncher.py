...
def mk_file_type(pth, title, md5sum):
    ...
    :return:
    """
    rel_to = os.path.abspath(os.path.join(bbc.get_bedbuncher_output_path(), os.pardir,os.pardir))
...
def main():
    ...
    for files in search_results:
        ...
        if not os.path.isabs(bedfile_path):
            bedfile_path = os.path.realpath(os.path.join(bbc.get_bedbuncher_output_path(), os.pardir,os.pardir, bedfile_path))
